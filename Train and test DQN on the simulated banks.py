    ...
    for j in range(training_size) :
        ...
        if (j+1) % validation_interval == 0 :
            ...
            try :
                ...
                torch.save(eval_net, 'E:/DQN with '+ prior + ' prior for ' + bank_type +'related banks/dqn_' + prior + '_' + bank_type + '_' + str(bank_id) + '_gamma_' + str(gamma) + '.t7')
...
def TEST(theta_test,
    ...
    with torch.no_grad():
        ...
        dqn_data.to_csv('E:/DQN with '+ prior + ' prior for ' + bank_type +'related banks/records_' + bank_type + '_' + str(bank_id) +'_DQN_' + prior + '_gamma_' + str(gamma) + '.csv')
...
item_bank = np.array(pd.read_csv('E:/'+ bank_type +'related banks/item_bank_' + bank_type + '_' + str(bank_id) + '.csv')[['a','b','c']])
...
### Start testing
theta_test = np.array(pd.read_csv('E:/theta_true/theta_true_' + str(bank_id) + '.csv')['x'])
TEST(theta_test)


### Load the pre-trained model
eval_net = Net(input_size, first_hidden, second_hidden, action_space, dropout_rate).to(device)
eval_net = torch.load('E:/DQN with '+ prior + ' prior for ' + bank_type +'related banks/dqn_'+ prior + '_' + bank_type + '_' + str(bank_id) + '_gamma_' + str(gamma) + '.t7')
