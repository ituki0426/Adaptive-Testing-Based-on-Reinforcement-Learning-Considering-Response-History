...
DATA_DIR    = EXP001_DIR / "data" / "real_responses_ShinyItemAnalysis_dataMedical"
MODEL_DIR   = EXP001_DIR / "models"
RESULTS_DIR = EXP001_DIR / "results" / "ShinyItemAnalysis_dataMedical"
...
def TRAIN(gamma,
    ...
    for j in range(training_size) :
        ...
        if (j+1) % validation_interval == 0 :
            ...
            validation_id = np.random.choice(training_size, validation_size, replace=False)
            ...
            for i in range(test_length):
                ...
                reward = np.transpose(FI(item_bank[action,], state[-1]))
            ...
            try :
                ...
                torch.save(eval_net, MODEL_DIR / ('dqn_gamma_' + str(gamma) + '_1.t7'))

            if (abs(step_valid[-1,1]) < 0.05) & (step_valid[-1,2] < 0.2) & (step_valid[-1,3] < 0.2) :
                best_valid = np.vstack((best_valid, step_valid[-1,1:]))
                torch.save(eval_net, MODEL_DIR / ('dqn_gamma_' + str(gamma) + '_' + str(best_valid.shape[0]) + '.t7'))
...
def TEST(theta_test,
    ...
    with torch.no_grad():
        ...
        if testing_size is None:
            testing_size = test_resp.shape[0]
        ...
        dqn_data.to_csv(RESULTS_DIR / ('records_real_bank_responses_DQN_gamma_' + str(gamma) + '.csv'), index=False)
...
network_id = 1
eval_net = torch.load(MODEL_DIR / ('dqn_gamma_' + str(gamma) + '_' + str(network_id) + '.t7'), weights_only=False)
