#!/usr/bin/env Rscript
# EXP027: MFI with EAP using uniform prior U(-4, 4)
# Based on src/Test MFI on the simulated bank.R, with priorDist changed from
# "norm" N(0,1) to "unif" U(-4,4) in both nextItem and thetaEst calls.
...
output_dir   <- get_env("OUTPUT_DIR", file.path(root_dir, "EXP027", "results"))
...
records_path <- file.path(results_dir, sprintf("records_%s_%d_MFI_EAP_unif%s.csv", bank_type, bank_id, output_suffix))
summary_path <- file.path(results_dir, sprintf("summary_%s_%d_MFI_EAP_unif%s.csv", bank_type, bank_id, output_suffix))
