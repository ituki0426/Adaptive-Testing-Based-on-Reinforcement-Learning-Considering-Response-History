...
2. **同じ推定値でも不確実性が異なる**: 例えば θ̂=0.5 でも、5問解いた後と20問解いた後では推定の確からしさが全く異なる。スカラーの推定値だけでは、この不確実性の違いを区別できない

### 本研究の提案

CAT の項目選択問題を**部分観測マルコフ決定過程（POMDP）**として再定式化し、回答履歴を状態として利用する。
...
- `seminar_docs/` — ゼミ発表資料・進捗メモ
  - `ゼミ.md` — 研究テーマ全体の導入資料。CD-CAT の基本概念、アトリビュートプロフィール、研究目的の説明に使う
  - `0417_ゼミ.md` — CD-CAT と強化学習の初期調査メモ。参考論文、基礎概念、質問事項の整理が中心
  - `ゼミ_2週目.md` — Wang et al. (2024) の読み下し資料。IRT-CAT を MDP として定式化する元論文の要点整理
  - `ゼミ3週目.md` — 再現実験の初期比較資料。MFI・DQN・DRQN・DDRQN の実装比較と代替データでの性能確認
  - `ゼミ4週目.md` — belief 入力系の拡張メモ。DBQN / ARDQN の実装方針と belief の離散化表現を説明
  - `ゼミ5週目.md` — シミュレーション実験の中間報告。DRQN / ADRQN / DDRQN の比較とアイテム情報入力の効果検証
  - `ゼミ6週目.md` — 実データ 5-fold CV の評価メモ。RMSE・MAE・Bias・相関の fold 別可視化が中心
  - `ゼミ7週目.md` — 項目選択挙動の可視化資料。ヒートマップ、ユニーク問題数、選択軌跡で方策の振る舞いを確認
  - `ゼミ_改善版.md` — 研究計画の再構成スライド。CD-CAT×RL の研究ギャップ、POMDP 定式化、報酬設計候補を整理
...
- `papers/` — 参考論文 PDF・Markdown 版
  - 対応する Markdown 版（`.md`）がある論文は、内容把握や実装参照では Markdown を優先し、必要に応じて PDF と照合する
  - `s13428-024-02498-x.md` — Wang et al. (2024) An adaptive testing item selection strategy via a deep reinforcement learning approach。IRT-CAT を MDP として定式化した DQN ベース項目選択法の元論文
  - `1507.06527v4.md` — Hausknecht & Stone (2015) Deep Recurrent Q-Learning for Partially Observable MDPs。DRQN の原典で、POMDP に対する LSTM 付き DQN の基本設計を確認する資料
  - `1704.07978v6.md` — Zhu et al. On Improving Deep Reinforcement Learning for POMDPs。ADRQN 系の拡張を扱い、行動と観測の組み合わせ入力の参考になる
  - `363_report.md` — Maxim Egorov (2015) Deep Reinforcement Learning with POMDPs。belief ベース入力と history ベース入力を比較する短報
  - `IPSJ-GPWS2018034.md` — Oh Hyunwoo, 金子知適 (2018) LSTM の初期状態の学習による DRQN の改善。更新開始時 hidden state の学習による DRQN 改善を扱う
  - `s11336-009-9123-2.md` — When Cognitive Diagnosis Meets Computerized Adaptive Testing: CD-CAT。CD-CAT の基礎枠組みと KL / エントロピー系項目選択基準を押さえる論文
  - `10.1177_0013164418790634.md` — Lin & Chang (2019) Item Selection Criteria With Practical Constraints in CD-CAT。属性バランスと露出制御を含む制約付き CD-CAT 項目選択を扱う
  - `reinforcement-learning-applied-to-adaptive-classification-5badyfmq0l.md` — Nurakhmetov, Reinforcement Learning Applied to Adaptive Classification Testing。適応的分類テストへの RL 導入事例
- `img/` — 図
...
- 論文を Markdown 化する場合は、原則として `papers/<PDFファイル名>.md` の対応を保つ
