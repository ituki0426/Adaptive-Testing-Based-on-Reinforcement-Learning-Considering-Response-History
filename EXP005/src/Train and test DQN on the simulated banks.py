...
EXP005_DIR = Path(__file__).resolve().parents[1]
MODEL_DIR = EXP005_DIR / "models"
RESULTS_DIR = EXP005_DIR / "results"
...
### The function to calculate uncertainty from the administered items' test information for the training phase (scalar)
### Uses 1/(1+I) mapping: (0,1] without clipping, preserving early-step uncertainty differences
def POSTERIOR_VAR(item_paras, theta):
    info = FI(item_paras, theta).sum()          # I(θ̂) = Σ_k FI_k(θ̂)
    return float(1.0 / (1.0 + info))
...
def TRAIN(gamma,
    ...
    for j in range(training_size) :
        ...
        state = np.concatenate((theta_hat, [prior_var]))    # [θ̂, 初期不確実性] = [θ̂, 1.0]
        ...
        for i in range(test_length) :
            ...
            next_state = np.concatenate((next_theta, [next_var]))   # [θ̂', 1/(1+I)]
        ...
        if (j+1) % validation_interval == 0 :
            ...
            state      = np.vstack((theta_hat0, var0))       # 行0 = θ̂, 行1 = 1/(1+I)
            item_id = np.array([])
            ...
            for i in range(test_length):
                ...
                var_0 = POSTERIOR_VAR_TEST(item_bank[item_id], theta_0)
            ...
            model_path = MODEL_DIR / f"dqn_ivar_{prior}_{bank_type}_{bank_id}_gamma_{gamma}.t7"
            try :
                best_valid
            except NameError :
...
def TEST(theta_test,
    ...
    with torch.no_grad():
        ...
        for i in range(test_length):
            ...
            var_0 = POSTERIOR_VAR_TEST(item_bank[item_id], theta_0[0])
        ...
        dqn_data.to_csv(RESULTS_DIR / f"records_{bank_type}_{bank_id}_DQNivar_{prior}_gamma_{gamma}.csv", index=False)
...
input_size = 2          # [θ̂, 1/(1+I)]
prior_var = 1.0         # 初期不確実性 = 1/(1+0) = 1.0
first_hidden = 50
second_hidden = 30
dropout_rate = 0
...
eval_net = Net(input_size, first_hidden, second_hidden, action_space, dropout_rate).to(device)
eval_net = torch.load(MODEL_DIR / f"dqn_ivar_{prior}_{bank_type}_{bank_id}_gamma_{gamma}.t7")
