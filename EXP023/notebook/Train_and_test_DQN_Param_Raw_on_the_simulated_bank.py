...
#
# EXP020 の EAP 能力推定を引き継ぎ、項目 ID ごとの出力を、
# `[theta_hat_EAP, t/L, a, b, c] -> Q` の共有 MLP に置き換える。
#
...
EXP023_DIR = ROOT / "EXP023"
MODEL_DIR = EXP023_DIR / "models"
RESULTS_DIR = EXP023_DIR / "results"
...
class Config:
    ...
    # Training (inherited from EXP020 unless noted otherwise)
    test_length: int = 40
    gamma: float = 0.1
    ...
    def validate(self) -> None:
        if self.state_size != 2:
            raise ValueError("EXP023 requires state_size=2: [theta_hat, t/L].")
        if self.item_feature_size != 3:
            raise ValueError("EXP023 Raw requires item_feature_size=3: [a, b, c].")
...
stem = (
    f"uncor_{cfg.bank_id}_DQN_Param_Raw_{cfg.prior}_gamma_{cfg.gamma}_seed_{cfg.seed}"
)
model_path = (
    MODEL_DIR
    / f"dqn_param_raw_{cfg.prior}_uncor_{cfg.bank_id}_gamma_{cfg.gamma}_seed_{cfg.seed}.pt"
)
torch.save(
    {
